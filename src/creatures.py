...
class Property(object):
    ...
    def __init__(self, 
        ...
        self.speed = speed
        self.towardFoodSpeed = towardFoodSpeed
    ...
    def inherit(self, mate):
        """
        舊版 inherit 邏輯:
            - 重組: 丟硬幣 (random < 0.5)，正面選自己(self)，反面選配偶(mate)，令其為 p 
        ...
        newP.towardFoodSpeed = self.mutation(p.towardFoodSpeed)
        """

        newP = self.__class__()

        # 重組特徵
        newP.hungerSpeed = self._pick_gene(self.hungerSpeed, mate.hungerSpeed)
        newP.matingDesireSpeed = self._pick_gene(self.matingDesireSpeed, mate.matingDesireSpeed)
        newP.speed = self._pick_gene(self.speed, mate.speed)
        newP.detectionRange = self._pick_gene(self.detectionRange, mate.detectionRange)
        newP.towardFoodSpeed = self._pick_gene(self.towardFoodSpeed, mate.towardFoodSpeed)      
        # 固定屬性
        newP.interactiveRange = self.interactiveRange
        newP.foodAmount = self.foodAmount

        newP.hungerLevel = 100 
        newP.matingDesireLevel = 0

        newP.hungerLevel = 100 
        newP.matingDesireLevel = 0

        # 突變
        newP.hungerSpeed = self.mutation(newP.hungerSpeed)
        newP.matingDesireSpeed = self.mutation(newP.matingDesireSpeed)
        newP.speed = self.mutation(newP.speed)
        newP.detectionRange = self.mutation(newP.detectionRange)
        newP.towardFoodSpeed = self.mutation(newP.towardFoodSpeed)

        return newP

    def _pick_gene(self, gene_a, gene_b): # 父母方各 50%
    ...
    def mutation(self, val):
        ...
        """
        # 根據 MUTATION_RATE 判斷是否發生突變
        if random.random() < MUTATION_RATE:
            mu = val
            sigma = val * MUTATION_SCALE 
...
class Creature(pygame.sprite.Sprite):
    ...
    def seekingFood(self, foods):
        for food in foods:
            ...
            if distance < self.property.detectionRange:
                self.speed = self.property.towardFoodSpeed
    ...
    def get_gene_color(self):
        # 取得當前基因相對於預設值的比例
        speed_ratio = self.property.speed / self.property.__init__.__defaults__[4] # 預設 SPEED
        sense_ratio = self.property.detectionRange / self.property.__init__.__defaults__[5] # 預設 DETECTION
        
        if speed_ratio > 1.2 and speed_ratio > sense_ratio:
            return (255, 50, 50) # 紅色：速度型 (跑得比平常快 20% 以上)
        elif sense_ratio > 1.2 and sense_ratio > speed_ratio:
            return (50, 50, 255) # 藍色：感知型 (看得比平常遠 20% 以上)
        else:
            return None 

    def draw(self, screen):
        color = self.get_gene_color()
        if color:
            # 畫一個半透明的圓圈在腳下 (需要另外建立 surface 才能支援 alpha 透明度)
            s = pygame.Surface((self.size * 2, self.size * 2), pygame.SRCALPHA)
            pygame.draw.circle(s, (*color, 100), (self.size, self.size), self.size // 1.5) # 100 是透明度
            screen.blit(s, (self.rect.centerx - self.size, self.rect.centery - self.size))
